...
#define MINIMUM_NUMBER_OF_ARGUMENTS 4
...
void monitor_files(char** file_names, char** file_paths, int* monitoring_frequencies, int files_number, int monitor_max_time, enum mode* m)
{
...
int parse_initial_arguments(int argc, char* argv[], char** list_file_name, int* monitor_max_time, enum mode* m)
{
...
int main(int argc, char* argv[])
{
    ...
    if(parse_initial_arguments(argc, argv, &list_file_name, &monitor_max_time, m) != 0)
        return 1;
    ...
    monitor_files(file_names, file_paths, monitoring_frequencies, lines_number, monitor_max_time, m);
